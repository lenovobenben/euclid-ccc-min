...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏。按相同几何对象去重后，Mannheim 正规八解先降至 \((18,61)\)，利用四条接触弦恒过根心又降至 \((18,57)\)。目标圆心线先把每对后缀从 8 E 降为 7 E；有限 \(K'\) 对角点再将正规后缀降为 6 E。两个精确标号分支进一步排除三重外切根唯一的 19 E 简单合并角色。当前正规子域的两个 profile 为 **\((18,49)\)**，完整上界为 **\((18,55)\)**。双 \(K'\) 平行夹具，以及先行简单合并与双 \(K'\) 平行同现的夹具，都精确达到当前 55 E 八解程序。
...
| Mannheim 全分支有限 \(K'\) 程序 | **18 / 55 E** | 一般 CCC 的单解 / 八解 | 单解用两个精确标号排除昂贵合并角色；八解的全部修复分支均覆盖 |
...
\((18,49)\)。两个精确标号分支证明完整 \(D_8\) 中至少一个
\(P_0\) 程序避开昂贵的 \(y=w\) 角色，所以两个完整 profile 的当前
上界为 \((18,55)\)。单解下一步应直接攻击 4 E 平行前缀、5 E 接触弦
核心或 4 E 定心后缀，寻找 17 E 程序。
