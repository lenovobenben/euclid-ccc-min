...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏。按相同几何对象去重后，Mannheim 正规八解先降至 \((18,61)\)；再利用四条接触弦恒过根心，精确降至 **\((18,57)\)**。根心恒为有限圆外点，且可由恒不重合的 \(P_0,P_2\) 两条接触弦取得。把全部局部修复接入共点程序后，完整上界为 **\((19,61)\)**。统一有序分支证书已经完成；旧双 \(K'\) 夹具按固定顺序恰为 49 条线、12 个圆，确认当前 61 E 程序可以饱和。
...
的内部复用量。先行 \(P_0,P_2\) 的任一修复分支各至多 13 E；取得根心
后，后行 \(P_1,P_3\) 各至多用 3 E 恢复接触弦、8 E 完成两个目标。
所以完整 \(D_8\) 的八解结构上界为 61 E。完整步骤与证明见
...
| Mannheim 全分支结构候选 | **19 / 61 E** | 一般 CCC 的单解 / 八解 | 共点、退化分类和统一有序分支证书完成；严格双 \(K'\) 夹具饱和当前程序 |
| Mannheim 正规共点分支 | **18 / 57 E** | 无五线块构造退化的 CCC 子域 | \(S\) 有限且 \(P_0,P_2\) 恒不重合；八解复用量 127 |
| Mannheim 双 \(K'\) 共点夹具 | 18 / 61 E | 严格双平行夹具 | 精确重放；比旧逐块程序少四个辅助圆 |
...
项目 E。换入合法平行宏、合并相同对象并利用四接触弦共点后，正规分支
为 \((18,57)\)；全部 \(D_8\) 分支的结构候选为 \((19,61)\)。统一有序
证书已经完成。公共前缀后 1--3 E 直接取得根心的多夹具搜索零命中；
下一步应缩短双目标后缀或先行平行块，并继续与热尔岗、反演路线按八解
profile 比较。
