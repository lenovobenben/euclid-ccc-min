...
4. 现已用一个合法的 6 E 双垂线程序替换该非法宏，得到项目规则下的 Mannheim 候选 \((E_{\mathrm{ext}},E_{\mathrm{all}})=(20,67)\)。每一步已合法，方向稳定性、构造特有退化和机器证书尚未完成，因此暂不称为已验证上界。
...
5. 合法双垂线替代把单目标候选提高到 20 E；仍可能存在少于 6 E 或能与后续对象共享的平行方向构造。
...
项目内推导的双垂线程序先用 \(\operatorname{Circle}(O_1,O_3)\) 与 \(\operatorname{Circle}(O_2,O_3)\) 得到 \(O_3\) 关于 \(O_1O_2\) 的镜像，从而画第一条垂线；再用该垂线与 \(\Gamma_3\) 的两个直径端点作第二条垂线。它使用 4 个圆和 2 条线，共 6 E，无任意点和距离搬运。

这比 Lemoine 的 3 对象宏多 3 E，因此得到：

\[
E_{\mathrm{candidate,ext}}=20,
\qquad
E_{\mathrm{candidate,pair}}=24,
\qquad
E_{\mathrm{candidate,all}}=67.
\]

八个目标各有 20 个宏级祖先，联合祖先为 67，按设计文档定义的复用量是 \(8\times20-67=93\)。完整步骤与证明见 [Mannheim 复原文档](MANNHEIM.md)。这些数字是本项目的新推导，不是历史文献原报，也尚未通过一般参数证书。
...
| Mannheim 合法化候选 | 20 / 67 E | 一般 CCC 的单解 / 八解 | 基础操作合法；待一般参数证明与机器证书 |
...
Mannheim 的四个方向类已在精确校准实例上全部重放，八条批量线的第二交点、八个目标身份、相似中心祖先和宏级依赖集合均已固定。历史对象模型下，每个目标祖先数为 17，八解联合祖先数为 64，复用量为 72，完成轨迹为 \((17,64)\)。换入合法双垂线宏后，候选轨迹为 \((20,67)\)，复用量为 93。下一步应搜索更短或可共享的平行方向构造，并完成 Mannheim 路线的一般参数证明与机器证书；随后再与热尔岗或反演路线按对应 profile 比较。
