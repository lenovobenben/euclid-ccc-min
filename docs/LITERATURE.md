...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏。进一步按相同几何对象去重后，Mannheim 正规八解为 \((18,61)\)；覆盖全部构造退化的当前结构上界为 \((19,65)\)。一般参数接触弦证明、退化分类和各分支精确重放已经完成，统一有序分支证书仍待整理。
...
\[
E_{\mathrm{regular,all}}=61,
\qquad
8\times18-61=83
\]

的内部复用量。构造退化中，简单合并模块因两条强制目标线复用而至多
12 E；其它模块至多 13 E，所以完整 \(D_8\) 结构上界为 65 E。一个
双 \(K'\) 平行的严格夹具恰使当前程序使用 65 个不同对象。完整步骤与
证明见 [Mannheim 复原文档](MANNHEIM.md)。这些数字是本项目的新推导，
不是历史文献原报。
...
| Mannheim 全分支结构候选 | **19 / 65 E** | 一般 CCC 的单解 / 八解 | 一般参数证明及分支重放完成；待统一有序证书 |
| Mannheim 正规分支 | 18 / 61 E | 无构造退化的 CCC 子域 | 精确对象去重；八解复用量 83 |
...
项目 E。换入合法平行宏且合并相同对象后，正规分支为 \((18,61)\)；
全部 \(D_8\) 分支的当前结构候选为 \((19,65)\)。下一步应把各分支整理
为统一有序证书，针对达到 65 E 的双 \(K'\) 平行族寻找替代，再与热尔岗
或反演路线按八解 profile 比较。
