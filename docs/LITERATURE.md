...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏。按相同几何对象去重后，Mannheim 正规八解先降至 \((18,61)\)，利用四条接触弦恒过根心又降至 \((18,57)\)。进一步利用固定方向类的两个目标圆心与根心共线，每对目标后缀从 8 E 降为 7 E。当前正规分支精确为 **\((18,53)\)**，完整上界为 **\((19,57)\)**。旧双 \(K'\) 夹具现为 45 条线、12 个圆，确认当前完整 57 E 程序可以饱和。
...
后缀降为 7 E。正规分支因此降到 53 E；计入简单合并的强制冗余批量线
后，完整 \(D_8\) 上界降到 57 E。完整步骤与证明见
...
| Mannheim 全分支圆心线程序 | **19 / 57 E** | 一般 CCC 的单解 / 八解 | 共点、退化分类和统一有序证书完成；严格双 \(K'\) 夹具饱和当前程序 |
| Mannheim 正规圆心线分支 | **18 / 53 E** | 无五线块构造退化的 CCC 子域 | 每对目标后缀由 8 E 降为 7 E；精确重放 |
| Mannheim 双 \(K'\) 圆心线夹具 | 18 / 57 E | 严格双平行夹具 | 45 线 + 12 圆；当前完整程序的精确饱和实例 |
...
目标圆心线后，正规分支为 \((18,53)\)，全部 \(D_8\) 分支为
\((19,57)\)。统一有序证书已经完成。下一步应寻找四条圆心线之间的
跨方向复用，并继续与热尔岗、反演路线按八解 profile 比较。
