...
| Mannheim 单目标裁剪 | 21 个已画对象 | 一般 CCC 的同号分支 | 单解候选；含非法宏，非 E |
...
下一阶段应同时严格展开 Mannheim 的 64 对象八解方案和 21 对象单解裁剪，并为同一程序记录两个完成时刻。第一处硬缺口是“过第三圆心作平行线”的 3 对象宏：需要在无任意点、塌圆规的当前状态中找到合法替代并计算真实边际 E。随后以八个目标的依赖集合交叠衡量复用，再与 Viète、Gergonne 或反演路线按对应 profile 比较。
