...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏。按相同几何对象去重后，Mannheim 正规八解先降至 \((18,61)\)；再利用四条接触弦恒过同一点，精确降至 **\((18,57)\)**。该共点恒为有限点，且可由恒不重合的 \(P_0,P_2\) 两条接触弦取得。此前逐块修复达到 65 E 的双 \(K'\) 平行夹具也降为 61 E。覆盖全部构造退化的保守结构上界暂仍为 \((19,65)\)，因为各修复分支尚未接入共点程序。
...
的内部复用量。旧逐块分支分析仍给出完整 \(D_8\) 的保守 65 E 上界，
但原来达到它的严格双 \(K'\) 平行夹具已由共点程序降为 61 E。完整步骤
与证明见 [Mannheim 复原文档](MANNHEIM.md)。这些数字是本项目的新推导，
不是历史文献原报；共点退化分类完成前还不能把全分支上界直接写成 61 E。
...
为 \((18,57)\)；全部 \(D_8\) 分支的保守结构候选仍为 \((19,65)\)。
下一步应把先行和后行方向类的合并、平行修复接入共点程序，整理为统一
有序证书，再与热尔岗或反演路线按八解 profile 比较。
