...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏，得到项目规则下的 Mannheim 候选 \((E_{\mathrm{ext}},E_{\mathrm{all}})=(18,65)\)。每一步已合法，方向稳定性、构造特有退化和机器证书尚未完成，因此暂不称为已验证上界。
...
八个目标各有 18 个宏级祖先，联合祖先为 65，按设计文档定义的复用量是 \(8\times18-65=79\)。完整步骤与证明见 [Mannheim 复原文档](MANNHEIM.md)。这些数字是本项目的新推导，不是历史文献原报，也尚未通过一般参数证书。
...
| Mannheim 合法化候选 | 18 / 65 E | 一般 CCC 的单解 / 八解 | 基础操作合法；待一般参数证明与机器证书 |
...
Mannheim 的四个方向类已在精确校准实例上全部重放，八条批量线的第二交点、八个目标身份、相似中心祖先和宏级依赖集合均已固定。历史对象模型下，每个目标祖先数为 17，八解联合祖先数为 64，复用量为 72，完成轨迹为 \((17,64)\)。换入合法 4 E 镜像—对径宏后，候选轨迹为 \((18,65)\)，复用量为 79。3 E 多实例枚举尚无命中，但没有形成下界。下一步应寻找与后续 Mannheim 对象共享的平行方向构造，并完成一般参数证明与机器证书；随后再与热尔岗或反演路线按对应 profile 比较。
