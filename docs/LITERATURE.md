...
4. 现已用一个合法的 4 E 镜像—对径程序替换该非法宏。按相同几何对象去重后，Mannheim 正规八解先降至 \((18,61)\)；再利用四条接触弦恒过根心，精确降至 **\((18,57)\)**。根心恒为有限圆外点，且可由恒不重合的 \(P_0,P_2\) 两条接触弦取得。此前逐块修复达到 65 E 的双 \(K'\) 平行夹具也降为 61 E。把全部局部修复接入共点程序后，完整结构上界正式收紧为 **\((19,61)\)**。
...
| Mannheim 全分支结构候选 | **19 / 61 E** | 一般 CCC 的单解 / 八解 | 共点、根心、退化分类与分支成本证明完成；待统一有序证书 |
...
为 \((18,57)\)；全部 \(D_8\) 分支的结构候选为 \((19,61)\)。下一步
应把共点分支整理为统一有序证书，再与热尔岗或反演路线按八解 profile
比较。
